...
void emit_file(std::unique_ptr<llvm::Module> _module,
  ...
  // deprecated for optimization, but is still the only complete API for codegen
  // as of work-in-progress LLVM14. At the time of this comment (Dec 2021),
  // there is no firm plan as to when codegen will be fully available in the new
  // PassManager, so don't worry about this 'legacy' tag until there's any
  // indication that the old APIs start breaking.
  //
  // See:
