...
void CodeGen_LLVM::init_context() {
  ...
  /*
  // TODO: are these even allowed? Halide adds these, I don't think they're safe
  though. fast_flags.setNoNaNs(); fast_flags.setNoInfs();
...
void CodeGen_LLVM::optimize_module() {
  ...
  if (tm->isPositionIndependent()) {
    // Add a pass that converts lookup tables to relative lookup tables to make
    // them PIC-friendly. See https://bugs.llvm.org/show_bug.cgi?id=45244
  ...
  for (auto &function : *module) {
    ...
    if (false) { // get_target().has_feature(Target::TSAN)
      ...
      // is not analyzed instruction by instruction. if
      ...
      //       detected features;
      //       // we should really fix it properly inside the implementation,
...
void CodeGen_LLVM::visit(const BinOp *node) {
  ...
  if (node->a.type().is_float()) {
    ...
    default: {
      internal_error << "Unimplemented BinOp lowering for signed integer: "
                     << Expr(node);
    }
    }
    ...
    default: {
      internal_error << "Unimplemented BinOp lowering for unsigned integer: "
                     << Expr(node);
    }
    }
...
void CodeGen_LLVM::visit(const Cast *node) {
  ...
  if (src.is_int_or_uint() && dst.is_int_or_uint()) {
    value =
        builder->CreateIntCast(_value, llvm_dst, /* isSigned */ src.is_int());
  } else if (src.is_float() && dst.is_int()) {
    value = builder->CreateFPToSI(value, llvm_dst);
  } else if (src.is_float() && dst.is_uint()) {
    // TODO: Halide has a weird corner case for uint1 -> float, but we don't use
    // uint1 as bools. so I think we can ignore this, and handle it explicitly
    // in bool -> float casts. Note: this has undefined behavior on overflow.
...
void CodeGen_LLVM::visit(const Build *node) {
  ...
  if (build_type->isVectorTy()) {
    ...
    if (defaults.empty() && values.empty()) {
      ...
      // Is order of codegen important? Default values must be constants (w/o
      // side effects), so I think no?
  ...
  if (value_type.is_scalar()) {
    ...
    if (is_dense) {
      ...
      for (int i = 0; i < value_type.lanes(); i++) {
        ...
        llvm::Value *v = builder->CreateExtractElement(expr, lane);
        llvm::Value *ptr =
            codegen_buffer_pointer(node->name, value_type.element_of(), idx);
        llvm::StoreInst *store = builder->CreateStore(v, ptr);
...
void CodeGen_LLVM::visit(const IfElse *node) {
  ...
  // TODO: we will support a switch statement, make sure to use Halide's codegen
  // for it!
...
llvm::Value *CodeGen_LLVM::codegen_write_loc(const ir::WriteLoc &loc) {
  ...
  if (frames.name_in_scope(loc.base)) {
    ...
    base =
        builder->CreateAlloca(base_type, /* arraysize ? */ nullptr, loc.base);
  ...
  for (const auto &value : loc.accesses) {
    if (std::holds_alternative<std::string>(value)) {
      internal_error << "TODO: implement field write access: "
                     << std::get<std::string>(value) << " in loc: " << loc;
    } else {
  ...
  // llvm::errs() << *base << "\n";
  // llvm::errs() << *ptr << "\n";
  // internal_assert(loc.accesses.empty()) << "TODO: implement codegen writeloc
  // for accesses: " << loc;
  return ptr;
}
