...
Expr IntImm::make(Type t, int64_t value) {
  ...
  if (infer_types) {
    ...
    // it's unlikely any compilers we care about will misbehave, but UBSan will
    // complain.
...
Expr FloatImm::make(Type t, double value) {
  ...
  if (infer_types) {
    ...
    case 64:
      break;
    default:
      internal_error << "FloatImm must be f16, f32, or f64, instead received: "
                     << t;
    }
  }
    ...
    if (a.type().is<Option_t>()) {
      if (b.type().is_bool()) {
        ...
        internal_assert(ir::equals(a.type().as<Option_t>()->etype, b.type()))
            << "Attempt to match types of option: " << a << " with: " << b
            << "failed.";
      ...
      if (a.type().is_bool()) {
        ...
        internal_assert(ir::equals(b.type().as<Option_t>()->etype, a.type()))
            << "Attempt to match types of option: " << b << " with: " << a
            << "failed.";
...
Expr Build::make(Type type, std::vector<Expr> values) {
  ...
  if (infer_types) {
    ...
    // TODO: if values.empty() or values.size() < expected_n, then assert type
    // has defaults!
    ...
    if (type.is<Vector_t>()) {
      internal_assert(values.empty() ||
          ...
          << "Build<Vector_t> with incorrect number of arguments, expected: "
          << type << " but received " << values.size() << " elements.";
      Type etype = type.as<Vector_t>()->etype;
      for (const auto &expr : values) {
        internal_assert(equals(expr.type(), etype))
            << "Build<Vector_t> requires uniform element type, expected: "
        ...
        if (field_count == value_count) {
          ...
          for (size_t i = 0; i < field_count; i++) {
            ...
            if (defaults.contains(fields[i].first)) {
              // No need to assert, the default should always be the correct
              // type for the struct.
              ...
              internal_assert(equals(fields[i].second, values[value_i].type()))
                  ...
                  << fields[i].second << " but received " << values[value_i]
                  << " of type " << values[value_i].type() << " for field "
                  << fields[i].first;
      ...
      internal_error << "Build::make with non-(vector, struct, option) type: "
                     << type;
    }
  }
...
Expr GeomOp::make(OpType op, Expr a, Expr b) {
  internal_assert(a.defined() && b.defined())
      << "GeomOp::make received undefined value: " << to_string(op) << " " << a
      << " " << b;
  ...
  if (infer_types) {
    if (op == SetOp::filter) {
      ...
      internal_assert(f->arg_types.size() == 1 &&
          ...
          << b.type().element_of() << " instead got " << a << " : " << a.type();
      node->type = b.type();
    ...
    for (size_t i = 0; i < args.size(); i++) {
      ...
      if (!args[i].type().defined()) {
        internal_assert(is_const(args[i]))
            << "Undefined type in function call for non-constant expression: "
            << args[i];
