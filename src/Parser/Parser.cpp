...
private:
      ...
      do {
        ...
        if (consume(Token::Type::ASSIGN)) {
          ...
          internal_assert(ir::is_constant_expr(_default))
              ...
              << _default << " for field " << field_name << " of element "
              << name;
  ...
  void parseExtern() {
    // TODO: what if an extern name-conflicts with a type or something? probably
    // should check conflicts for all symbols?
  ...
  void parseFunction() {
    ...
    if (peek().type != Token::Type::RPAREN) {
      // parse arg list
      do {
        // TODO: can we accept multiple args with one type here, as in element
        // definitions?
        ...
        if (consume(Token::Type::ASSIGN)) {
          ...
          internal_assert(ir::is_constant_expr(default_value))
              << "Function default values must be constants, received: "
              << default_value << " for argument " << arg_name << " of func "
              << name;
        }
        ...
        args.push_back(ir::Function::Argument{arg_name, std::move(type),
    ...
    // TODO: the syntax of -> type = expr is ugly, maybe disallow, and just do
    // inference?
    ...
    if (consume(Token::Type::LBRACKET)) {
      if (consume(Token::Type::LBRACKET)) {
        ...
        internal_assert(!template_types.empty())
            << "Template syntax expects type arguments, but did not receive "
               "any for name: "
            << name << " at line: " << token.lineBegin;
        internal_assert(peek().type == Token::Type::LPAREN)
            << "Template syntax supported only for function calls, found on "
               "name: "
            << name << " at line: " << token.lineBegin;
      } else {
        std::vector<ir::Expr> idxs = parseExprListUntil(Token::Type::RBRACKET);
        internal_assert(!idxs.empty())
            << "Indexing into array/vector expects at least one index for "
               "name: "
            << name << " at line: " << token.lineBegin;
      ...
      if (fields.empty()) {
        if (name == "cast") {
          ...
          internal_assert(type.is_float())
              << "eps takes only floating point template types, instead "
                 "received: "
              << type;
        ...
        internal_assert(template_types.empty())
            << name << " does not take template parameters, but received: "
            << template_types.size() << " at line: " << token.lineBegin;
        ...
        if (name == "abs") {
          ...
          if (args.size() == 1) {
            ...
            internal_assert(args.size() == 2)
                << "max() currently only supports binary and unary, instead "
                   "received: "
          ...
          if (args.size() == 1) {
            ...
            internal_assert(args.size() == 2)
                << "min() currently only supports binary and unary, instead "
                   "received: "
                << args.size() << " arguments at line " << token.lineBegin;
          ...
          internal_assert(args[1].is<ir::Build>())
              << "permute expects the second argument to be a list of indexes, "
                 "instead "
                 "received: "
              << args[1];
          ...
          if (program.funcs.contains(name)) {
            ...
            if (func->ret_type.defined()) {
              // Argument types are always required, but the return type could
              // not be.
              // TODO: does that mean we can pull this check out of the if
              // statement?
              ...
              for (size_t i = 0; i < func->args.size(); i++) {
                ...
                // types. That mixes type inference with parsing though, not
                // sure we want that.
            ...
            // TODO: if we allowed partially-defined types, we could make:
            // Fn(arg_types)
            // -> (undef) that would make type inference easier, but we'd have
            // to change a lot of the error handling in IR/Type.cpp For now,
            // leave ftype undefined in the else case
  ...
  ir::Type parseType() {
    ...
    // Now look for built-ins
      ...
      // TODO: this upper bound is arbitrary. Can't imagine needing a larger one
      // though?
      ...
      internal_assert(!etype.is_bool())
          << "Bonsai does not support option[bool] because the semantics are "
             "confusing.";
