...
struct Parser {
  ...
  private:
    ...
    Token expect(Token::Type type) {
        if (auto token = consume(type)) {
            ...
            internal_error << "Expected " << Token::tokenTypeString(type)
                           ...
                           << " at line: " << peek().lineBegin << ":" << peek().colBegin;
            return Token{};
        }
    }
    ...
    void parseElement() {
        ...
        do {
            ...
            do {
                ...
                if (consume(Token::Type::ASSIGN)) {
                    ...
                    internal_assert(ir::is_constant_expr(_default))
                        << "Field default values must be constants, received: " << _default
                        << " for field " << field_name << " of element " << name;
            ...
            for (const auto &field_name : names) {
                internal_assert(fields.cend() == std::find_if(fields.cbegin(), fields.cend(),
            ...
            if (consume(Token::Type::ASSIGN)) {
                ...
                internal_assert(ir::is_constant_expr(_default))
                    << "Field default values must be constants, received: " << _default
                    << " for typed fields " << " of element " << name;
                _default = constant_cast(type, _default);
                internal_assert(_default.defined());
                for (const auto &field_name : names) {
                    internal_assert(!defaults.contains(field_name))
                        << "Duplicate default value for field " << field_name << " of element "
                        << name << " individually marked: " << defaults[field_name]
        ...
        program.types[name] =
            defaults.empty() ? ir::Struct_t::make(name, std::move(fields))
                             : ir::Struct_t::make(name, std::move(fields), std::move(defaults));
    }

    void parseInterface() { internal_error << "TODO: implement parseInterface"; }

    void parseExtern() {
        // TODO: what if an extern name-conflicts with a type or something? probably should check
        // conflicts for all symbols?
        expect(Token::Type::EXTERN);
        const Token id = expect(Token::Type::IDENTIFIER);
        const std::string name = std::get<std::string>(id.value);
        internal_assert(program.externs.cend() ==
    ...
    void parseFunction() {
            ...
            do {
                ...
                if (consume(Token::Type::ASSIGN)) {
                    ...
                    internal_assert(ir::is_constant_expr(default_value))
                        << "Function default values must be constants, received: " << default_value
                        << " for argument " << arg_name << " of func " << name;
                }

                add_type_to_frame(arg_name, type,
                                  /* mutable */ false); // TODO: handle mutable args in functions.
                args.push_back(
                    ir::Function::Argument{arg_name, std::move(type), std::move(default_value)});
            } while (consume(Token::Type::COMMA));
        }
        ...
        program.funcs[name] =
            std::make_shared<ir::Function>(name, std::move(args), std::move(ret_type), ir::Stmt());
        ...
        internal_assert(!program.funcs[name]->body.defined())
            << "Woah, how did " << name << " get a function body before being parsed?";
    ...
    ir::Stmt parseStmt() {
        if (peek().type == Token::Type::LSQUIGGLE) {
            ...
            internal_assert(!consume(Token::Type::ELIF))
                << "TODO: implement elif parsing for line: " << peek().lineBegin;
    ...
    ir::Stmt parseAssign(ir::WriteLoc loc) {
        ...
        if (loc.type.defined() && value.type().defined()) {
            internal_assert(ir::equals(loc.type, value.type()))
                << "Mismatching assignment: " << loc << " has type: " << loc.type << " but "
    ...
    ir::Expr parseBinOpWithPrecedence(F parseSubExpr, const std::array<BinOperator, N> &ops) {
        ...
        while (true) {
            ...
            auto it = std::find_if(ops.begin(), ops.end(),
                                   [this](const auto &op) { return consume(op.token); });
    ...
    ir::Expr parseMulDivMod() {
        return parseBinOpWithPrecedence<3>([this]() { return parseBaseExpr(); },
                                           {{
    ...
    ir::Expr parseAddSub() {
        return parseBinOpWithPrecedence<2>([this]() { return parseMulDivMod(); },
                                           {{
    ...
    ir::Expr parseRels() {
        return parseBinOpWithPrecedence<4>([this]() { return parseAddSub(); },
                                           {{
    ...
    ir::Expr parseAnd() {
        return parseBinOpWithPrecedence<1>([this]() { return parseEqs(); },
                                           {{{ir::BinOp::And, Token::Type::AND}}});
    }

    // xor_expr := or_expr ('^' or_expr)*
    ir::Expr parseXor() {
        return parseBinOpWithPrecedence<1>([this]() { return parseAnd(); },
    ...
    ir::Expr parseOr() {
        return parseBinOpWithPrecedence<1>(
            // TODO: logical and!
            [this]() { return parseXor(); }, {{{ir::BinOp::Or, Token::Type::OR}}});
    }

    // base_expr := '(' expr ')' | name (('.' field) | ('[' index (, index)* ']' | () ) | lambda
    // args? : expr
        ...
        if (consume(Token::Type::LBRACKET)) {
            if (consume(Token::Type::LBRACKET)) {
                ...
                internal_assert(!template_types.empty())
                    << "Template syntax expects type arguments, but did not receive any for name: "
                    << name << " at line: " << token.lineBegin;
                internal_assert(peek().type == Token::Type::LPAREN)
                    << "Template syntax supported only for function calls, found on name: " << name
                    << " at line: " << token.lineBegin;
            } else {
                std::vector<ir::Expr> idxs = parseExprListUntil(Token::Type::RBRACKET);
                internal_assert(!idxs.empty())
                    << "Indexing into array/vector expects at least one index for name: " << name
                    << " at line: " << token.lineBegin;
            ...
            if (fields.empty()) {
                if (name == "cast") {
                    internal_assert(template_types.size() == 1)
                        << "cast() expects a template parameter, instead received: "
                        << template_types.size() << " at line: " << token.lineBegin;
                    internal_assert(args.size() == 1)
                        << "cast takes a single argument, received: " << args.size()
                        << " at line: " << token.lineBegin;
                    return ir::Cast::make(std::move(template_types[0]), std::move(args[0]));
                } else if (name == "eps") {
                    internal_assert(template_types.size() == 1)
                        << "eps() expects a template parameter, instead received: "
                        << template_types.size() << " at line: " << token.lineBegin;
                ...
                internal_assert(template_types.empty())
                    << name
                    << " does not take template parameters, but received: " << template_types.size()
                    << " at line: " << token.lineBegin;
                // TODO: generalize intrinsic parsing.
                if (name == "abs") {
                    internal_assert(args.size() == 1)
                        << "abs takes a single argument, received: " << args.size();
                    return ir::Intrinsic::make(ir::Intrinsic::abs, std::move(args));
                } else if (name == "sqrt") {
                    internal_assert(args.size() == 1)
                        << "sqrt takes a single argument, received: " << args.size();
                    return ir::Intrinsic::make(ir::Intrinsic::sqrt, std::move(args));
                } else if (name == "sin") {
                    internal_assert(args.size() == 1)
                        << "sin takes a single argument, received: " << args.size();
                    return ir::Intrinsic::make(ir::Intrinsic::sin, std::move(args));
                } else if (name == "cos") {
                    internal_assert(args.size() == 1)
                        << "cos takes a single argument, received: " << args.size();
                    return ir::Intrinsic::make(ir::Intrinsic::cos, std::move(args));
                } else if (name == "cross") {
                    internal_assert(args.size() == 2)
                        << "cross takes two arguments, received: " << args.size();
                    ...
                    internal_assert(args.size() == 2)
                        << "argmin takes two arguments, received: " << args.size();
                    return ir::SetOp::make(ir::SetOp::argmin, std::move(args[0]),
                                           std::move(args[1]));
                } else if (name == "filter") {
                    internal_assert(args.size() == 2)
                        << "filter takes two arguments, received: " << args.size();
                    return ir::SetOp::make(ir::SetOp::filter, std::move(args[0]),
                                           std::move(args[1]));
                } else if (name == "map") {
                    internal_assert(args.size() == 2)
                    ...
                    internal_assert(args.size() == 2)
                        << "product takes two arguments, received: " << args.size();
                    return ir::SetOp::make(ir::SetOp::product, std::move(args[0]),
                                           std::move(args[1]));
                } else if (name == "distance") {
                    internal_assert(args.size() == 2)
                        << "distance takes two arguments, received: " << args.size();
                    return ir::GeomOp::make(ir::GeomOp::distance, std::move(args[0]),
                                            std::move(args[1]));
                } else if (name == "intersects") {
                    internal_assert(args.size() == 2)
                        << "intersects takes two arguments, received: " << args.size();
                    return ir::GeomOp::make(ir::GeomOp::intersects, std::move(args[0]),
                                            std::move(args[1]));
                } else if (name == "contains") {
                    internal_assert(args.size() == 2)
                        << "contains takes two arguments, received: " << args.size();
                    return ir::GeomOp::make(ir::GeomOp::contains, std::move(args[0]),
                                            std::move(args[1]));
                } else if (name == "sum") {
                    internal_assert(args.size() == 1)
                        << "sum takes a single argument, received: " << args.size();
                    return ir::VectorReduce::make(ir::VectorReduce::Add, std::move(args[0]));
                } else if (name == "idxmin") {
                    internal_assert(args.size() == 1)
                        << "idxmin takes a single argument, received: " << args.size();
                    return ir::VectorReduce::make(ir::VectorReduce::Idxmin, std::move(args[0]));
                } else if (name == "idxmax") {
                    internal_assert(args.size() == 1)
                        << "idxmax takes a single argument, received: " << args.size();
                    ...
                    if (args.size() == 1) {
                        ...
                        internal_assert(args.size() == 2)
                            << "max() currently only supports binary and unary, instead received: "
                            << args.size() << " arguments at line " << token.lineBegin;
                    ...
                    if (args.size() == 1) {
                        ...
                        internal_assert(args.size() == 2)
                            << "min() currently only supports binary and unary, instead received: "
                            << args.size() << " arguments at line " << token.lineBegin;
                        return ir::Intrinsic::make(ir::Intrinsic::min, std::move(args));
                    }
                } else if (name == "any") {
                    internal_assert(args.size() == 1)
                        << "any takes a single argument, received: " << args.size();
                    return ir::VectorReduce::make(ir::VectorReduce::Or, std::move(args[0]));
                } else if (name == "all") {
                    internal_assert(args.size() == 1)
                        << "all takes a single argument, received: " << args.size();
                    ...
                    internal_assert(args[1].is<ir::Build>())
                        << "permute expects the second argument to be a list of indexes, instead "
                           "received: "
                        << args[1];
                    ...
                    if (program.funcs.contains(name)) {
                        ...
                        if (func->ret_type.defined()) {
                            ...
                            for (size_t i = 0; i < func->args.size(); i++) {
                                ...
                                // TODO: we could push types down here, because we know the arg
                                // types. That mixes type inference with parsing though, not sure we
                                // want that.
                                internal_assert(!args[i].type().defined() ||
                                                ir::equals(func->args[i].type, args[i].type()))
                                    << "Argument " << i << " of call to function " << name
                                    << " on line " << token.lineBegin
                                    << " has incorrect type. Expected " << func->args[i].type
                                    << " but parsed: " << args[i].type();
                            }
                            ftype = ir::Function_t::make(func->ret_type, arg_types);
                        }
                        // TODO: if we allowed partially-defined types, we could make: Fn(arg_types)
                        // -> (undef) that would make type inference easier, but we'd have to change
                        // a lot of the error handling in IR/Type.cpp For now, leave ftype undefined
                        // in the else case
    ...
    std::vector<ir::Lambda::Argument> parseLambdaArgs() {
        ...
        do {
            ...
            internal_assert(!def.value.defined()) << "Lambdas cannot have default function values! "
                                                  << def.name << " assigned default: " << def.value;
            internal_assert(!def.mut)
                << "TODO: support mutable arguments in lambdas. Argument: " << def.name
                << " marked as mutable.";
    ...
    // TODO: allow `mut` flag!
    template <bool T_REQUIRED, bool E_CONST> NameDef parseNameDef(const bool expr_allowed) {
        NameDef def;
    ...
    ir::WriteLoc parseWriteLoc() {
        ...
        while ((peek().type == Token::Type::PERIOD) || (peek().type == Token::Type::LBRACKET)) {
            if (consume(Token::Type::PERIOD)) {
                const Token field = expect(Token::Type::IDENTIFIER);
                const std::string field_name = std::get<std::string>(field.value);
    ...
    ir::Type parseType() {
        ...
        // Now look for built-ins
            ...
            internal_assert(!etype.is_bool())
                << "Bonsai does not support option[bool] because the semantics are confusing.";
