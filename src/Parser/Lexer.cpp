        ...
        if (programStream.peek() == '_' || std::isalpha(programStream.peek())) {
            ...
            default: {
                ...
                if (programStream.peek() == '.') {
                    ...
                    if (!std::isdigit(programStream.peek())) {
                        std::stringstream errMsg;
                        errMsg << "unexpected symbol (expected digit for decimal) '"
                ...
                if (programStream.peek() == 'e' || programStream.peek() == 'E') {
                    ...
                    if (!std::isdigit(programStream.peek())) {
                        std::stringstream errMsg;
                        errMsg << "unexpected symbol (expected digit for exponent) '"
                               << (char)programStream.peek() << "'";
                        reportError(errMsg.str(), line, col);
                ...
                if (newToken.type == Token::Type::FLOAT_LITERAL) {
                    ...
                    std::cout << "parsed as:\n";
                    std::cout << "double? " << std::holds_alternative<double>(newToken.value)
                              << std::endl;
                    std::cout << "int? " << std::holds_alternative<int64_t>(newToken.value)
                              << std::endl;
                    std::cout << "uint? " << std::holds_alternative<uint64_t>(newToken.value)
                              << std::endl;
