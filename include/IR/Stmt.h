...
template <typename T> Stmt StmtNode<T>::mutate_stmt(Mutator *m) const {
...
template <> inline RefCount &ref_count<ir::IRStmtNode>(const ir::IRStmtNode *t) noexcept {
    return t->ref_count;
}

template <> inline void destroy<ir::IRStmtNode>(const ir::IRStmtNode *t) {
    delete t;
}
